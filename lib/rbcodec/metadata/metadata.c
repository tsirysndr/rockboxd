...
/* Raw stderr tracing for stream/metadata debugging (surfaces in logcat on
 * Android and in the terminal on desktop). Compiled out in the standalone
 * rockbox-metadata crate, which must not write to stderr. */
#ifdef RBMETA_QUIET
#define META_TRACE(...) do { } while (0)
#else
#define META_TRACE(...) fprintf(stderr, __VA_ARGS__)
#endif
...
static unsigned int probe_content_type_format(int fd)
{
    ...
    if (ct_len < 0) {
        META_TRACE("[metadata] probe_content_type: fd=%d stream_content_type returned %zd "
                "(no Content-Type header or HTTP error)\n", fd, ct_len);
        return AFMT_UNKNOWN;
    }
    META_TRACE("[metadata] probe_content_type: fd=%d got \"%s\"\n", fd, content_type);
    ...
    META_TRACE("[metadata] probe_content_type: fd=%d unrecognized type \"%s\"\n",
            fd, content_type);
    return AFMT_UNKNOWN;
}
#endif
...
bool get_metadata_afmt(struct mp3entry* id3, int fd, const char* trackname, int audio_fmt, int flags)
{
    ...
    META_TRACE("[metadata] get_metadata_ex: trackname=%s fd=%d\n", trackname, fd);
    const char *res_str = "\n";
    ...
    if (fd == -1)
    {
        ...
        META_TRACE("[metadata] get_metadata_ex: opened fd=%d for %s\n", fd, trackname);
        if (fd == -1)
        {
     ...
     * audio_fmt is the AFMT_ enum the caller derived from probe_file_format(trackname). */
    id3->codectype = audio_fmt;
    META_TRACE("[metadata] probe_file_format: %s -> codectype=%u\n", trackname, id3->codectype);
#if defined(SIMULATOR) || defined(APPLICATION)
    ...
    if (stream_is_http_fd(fd))
    {
        ...
        META_TRACE("[metadata] probe_content_type_format: fd=%d -> codectype=%u\n", fd, ct_codec);
    ...
    else if (!entry->parse_func(fd, id3))
    {
        ...
        META_TRACE("[metadata] ERROR: parse failed for %s (format %s)\n", trackname, entry->label);
    ...
    {
        META_TRACE("[metadata] parsed OK: %s length=%lu elapsed=%lu freq=%lu filesize=%lu codectype=%u first_frame_off=%lu\n",
