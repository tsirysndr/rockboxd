...
### Added
- `daemon`: **rsql filters and listening analytics** — a new `play_history` event log (one row per listen, with how much was actually heard) alongside the running `track_stats` counters, five SQL views (`v_most_played`, `v_most_skipped`, `v_never_played`, `v_recently_added`, `v_recently_played`) defined in the migration so HTTP (`/analytics/*`), gRPC, GraphQL and the sqlite3 CLI all see the same numbers, and the `rockbox-rsql` crate (ported from music-player) compiling RSQL/FIQL expressions like `playcount>5;lastplayed=lt=30d` to parameter-bound SQLite WHERE clauses — exposed at `POST /rsql/{tracks,albums,artists}` and via the new `FilterTracks` RPC / `filterTracks` GraphQL query
- `daemon`: **smart-playlist rsql expressions** — `RuleCriteria` gains an optional `rsql` field that, when set, *is* the filter (limit/sort still apply, mapped onto rsql sort keys); optional and defaulted so every stored playlist deserializes unchanged, with tests pinning the sort mapping and pre-`rsql` rules-JSON compatibility
- `daemon`: **key/BPM analysis** — `track.key` / `track.bpm` columns filled by the new `rockbox-analysis` crate (tag first: TKEY / TBPM / initialkey; correlation detection only for untagged files) via a background pass (one file a second, local files only, failures marked so they are not reattempted every boot), so `key==Am` and `bpm>=120` filter as expected
- `daemon`: **16-band spectrum meter and stored per-track waveforms** — the pcm meter grows a ladder of 16 one-pole low-passes at log-spaced cutoffs (40 Hz … 16 kHz) whose neighbouring-stage differences fall out as band-passes (integer Q16 in `firmware/pcm_meter.c`, f32 in `rockbox-playback`, bumped to 0.7.0), published as `repeated float bands` on `StreamLevels`; the analysis pass now keeps the 400-peak-byte waveform it always computed (`track.waveform` BLOB) and `GetTrackWaveform` serves it, generating **on demand** (`analyze_if_missing`) with the background pass ordered by `last_played DESC` so the music actually being listened to is analysed first
- `desktop`: **analytics tabs** (Most Played and Statistics in the sidebar — ranked play counts, stat cards, recently-played / most-skipped / never-played sections), a **waveform seek bar** (400 → 120 bars, peak-preserving), a **96-bar spectrum strip** along the fullscreen player's bottom edge (20 Hz band meter, per-band auto-gain, meter ballistics, falling peak caps, only computed while open, `v` toggles it), palette-result **context menus** with a mini-art header, and a truly centred, **responsive player bar** (only the VFD auto-hides on narrow windows — controls stay)
- `desktop`: **full-screen now-playing canvas** — click the artwork or press `f`; the sidebar collapses and the player bar picks up the same blurred-artwork wash so the two read as one surface, with drawers and dialogs still painting above it — and a **value arc on every knob**, with the four bipolar controls (balance, bass, treble, replaygain pre-amp) anchoring their arc at the neutral centre
- `desktop`: an **artist detail page** (albums grid + songs); palette album/artist results now open their pages instead of starting playback, and artist rows in the Artists tab open it too
- `desktop`: **five new builtin skins** — Nord, Oceanic, Tape, Phosphor, Parchment — ten now ship embedded, with tests asserting every bundled skin parses and names are unique and findable
- `desktop`: **play and shuffle buttons on the Liked tracks screen** — a header row with track count mirroring the artist/album detail transport
- `rocksky`: the remote-player websocket now **sends the codec** with now-playing frames so controllers show a format badge (stream URLs send nothing — their extension says nothing about the audio inside)
- `packaging`: macOS ships as **`Rockbox.app` in a DMG** instead of a bare CLI tarball — `desktop/package-macos.sh` builds the bundle (shared appiconset icon, Info.plist, ad-hoc signature, /Applications symlink), the plist carries `NSLocalNetworkUsageDescription` + `NSBonjourServices` (macOS 15+ silently denies mDNS to bundles without them), and the homebrew-tap cask installs the app with a quarantine-stripping postflight; install instructions added to the README

### Fixed
- `embed`: the embedded daemon never started the **Rocksky remote-player bridge** — only the standalone CLI did — so the desktop app was invisible in Rocksky's device picker; the bridge now starts after gRPC binds (no-op without a `~/.config/rockbox.org/token`)
- `stats`: **no duplicate listens** — listens are keyed on the actual track-id change rather than a current-track read that flickers to None, and Recently Played collapses to the latest listen per track (the raw log stays intact) — and **every real listen is recorded**: anything that produced audio (more than half a second heard) reaches `track_stats` / `play_history`, instead of only tracks over ten seconds heard for more than two
- `desktop`: the **like button works everywhere** — liked flags are patched into the live models in place (album detail, playlist detail, queue included, scroll position preserved); the daemon replies to `LikeTrack`/`UnlikeTrack` immediately and mirrors to Rocksky in a spawned task (measured 4.28 s / 1.94 s blocking before); the desktop detaches the call so a like no longer stalls play/pause behind it; and the icon updates optimistically with sequence-numbered, failure-rollback responses
- `desktop`: the **waveform seek bar showed nothing** — the analysis pass selected `key IS NULL` so tracks analysed before the waveform column existed were never backfilled (now `key IS NULL OR waveform IS NULL`, and a failed analysis stores an empty waveform rather than NULL so the same broken file is not retried every boot)
- `desktop`: the **queue drawer refreshes as soon as a play command lands** instead of waiting for the playlist stream (whose publisher can lag many seconds), the **VFD is gated on the player bar's effective width** (window minus sidebar) so it no longer pushes the volume knob off the right edge, the window **opens at the full 1200×780 layout**, and every context menu leads with a mini-art + title + artist header
- `daemon`: **raise `RLIMIT_NOFILE`** at startup — a running rockboxd holds ~314 descriptors while launchd-started processes on macOS inherit a 256-fd soft limit, and actix-server's unwrap on the resulting EMFILE aborted the whole process mid-boot
- `build`: repack `librockboxd.a` deterministically on macOS, vendor the desktop icon/font assets so macOS CI can build, and allow multiple definitions when linking `librockboxd.a` on Linux
