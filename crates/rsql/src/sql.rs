...
mod tests {
    ...
    fn nests_and_or_with_the_right_precedence() {
        ...
        assert_eq!(
            filter.sql,
            "(genre.name = ? COLLATE NOCASE OR (track.year > ? AND track.bitrate >= ?))"
        );
    }
    ...
    fn compiles_value_lists() {
        ...
        assert_eq!(filter.sql, "genre.name IN (?, ?)");
        assert_eq!(
            filter.params,
    ...
    fn negations_keep_rows_with_no_value() {
        assert_eq!(
            ...
            "(genre.name IS NULL OR genre.name <> ? COLLATE NOCASE)"
        );
        assert_eq!(
            compiled("genre=out=(rock)").sql,
            "(genre.name IS NULL OR genre.name NOT IN (?))"
        );
    }
    ...
    fn empty_tests_treat_a_blank_tag_as_absent() {
        assert_eq!(
            ...
            "(genre.name IS NULL OR genre.name = '')"
        );
        assert_eq!(
