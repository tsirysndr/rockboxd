...
/* Raw stderr tracing for stream debugging (visible in logcat / terminal).
 * Compiled out in the standalone rockbox-codecs crate, which must not
 * write to stderr. */
#ifdef RBCODEC_QUIET
#define AAC_TRACE(...) do { } while (0)
#else
#define AAC_TRACE(...) fprintf(stderr, __VA_ARGS__)
#endif
...
enum codec_status codec_run(void)
{
    ...
    if (codec_init()) {
        AAC_TRACE("FAAD: Codec init error\n");
        return CODEC_ERROR;
    }
    ...
    if (!qtmovie_read(&input_stream, &demux_res)) {
        AAC_TRACE("FAAD: File init error (qtmovie_read failed)\n");
        return CODEC_ERROR;
    }
