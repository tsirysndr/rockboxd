...
<CardGroup cols={3}>
  ...
  <Card title="Player" icon="play">
    Queue + transport, **shuffle / repeat**, crossfade, ReplayGain, the full
    **DSP chain** (EQ + presets, tone, crossfeed, surround, compressor, …),
    and `status()`. Needs an audio output device.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Tone & spatial" icon="wave-sine">
    ...
    mixing (mono / karaoke / swap) and custom stereo width.
  </Card>
